...
char mqttTopic[255], mqttMessage[255], timeSyncData[255];

bool result, timeSyncActive;
...
bool updateMQTT() {  // This must be run periodically to allow for the message callback to run
    ...
    if( millis() > mqttUpdateTime ) {
        client.loop();  // This forces the update of the MQTT functions
        mqttUpdateTime = millis() + 100;
    }    

    // Check to see if MQTT is still connected
    if ( client.state()==0 ) return true;
